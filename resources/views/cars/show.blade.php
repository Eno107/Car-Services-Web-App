...
@section('body')
    <div class="max-w-xl w-full bg-white shadow-md rounded-md overflow-hidden border" x-data="app()">
        <form method="POST" action="/{{ Auth::user()->name }}/cars/{{ $car->id }}">
            @csrf
            <div class="flex justify-between items-center px-5 py-3 text-gray-700 border-b">
                <h3 class="text-sm">Edit car</h3>

                <a href="/{{ Auth::user()->name }}/cars"
                    class="px-3 py-1 text-gray-700 text-sm rounded-md bg-gray-200 hover:bg-gray-300 focus:outline-none">Go
                    Back</a>
            </div>
            ...
            <div class="px-5 py-6 bg-gray-200 text-gray-700 border-b ">
                ...
                <div class="mt-1 mb-1 relative rounded-md shadow-sm">
                    ...
                    <input type="number" step="0.1" pattern="[0-9]\.[0-9]" name="size" id="size"
                        value={{ $car->engine_size }} placeholder="Enter Engine Size"
                ...
                <div class="mt-1 mb-1 relative rounded-md shadow-sm">
                    <span class="absolute inset-y-0 left-0 pl-3 flex items-center text-gray-600">
                        <svg fill="#000000" version="1.1" id="Capa_1" class="w-6 h-6" xmlns="http://www.w3.org/2000/svg"
                            xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 45.393 45.393" xml:space="preserve">
                            <g>
                                <g>
                                    <path
			...
			c1.688,0,3.057,1.367,3.057,3.057C36.807,35.746,35.438,37.113,33.75,37.113z" />
                    ...
                    <select id="make" name="make"
                        ...
                        @foreach ($makes as $make)
                            <option value="{{ $make->id }}" {{ $car->make->id == $make->id ? 'selected' : '' }}>
                                {{ $make->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mt-1 mb-1 relative rounded-md shadow-sm">
                    ...
                    <input type="number" step="1" pattern="[0-9]{4}" name="year" id="year"
                        placeholder="Enter Year of Production" value="{{ $car->year }}"
                        class="form-input w-full px-12 py-2 appearance-none rounded-md focus:border-indigo-600">
                ...
                <div class="mt-1 mb-1 relative rounded-md shadow-sm">
                    <span class="absolute inset-y-0 left-0 pl-3 flex items-center text-gray-600">
                        <svg fill="#000000" class="w-6 h-6" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg"
            ...
            <div class="flex justify-end items-center px-5 py-3">
                <button type="submit"
                    class="bg-gray-700 hover:bg-gray-800 text-white font-bold py-1 px-3 rounded text-sm main-button">Save
                    Edit</button>

            </div>
        </form>

        <form class="flex justify-end items-center px-5 py-3" method="POST" x-data="{ showNotification: false }">
            @csrf
            @method('DELETE')
            <button
                @if (isset($car->appointments) && count($car->appointments) > 0) @click="toggleNotification" 
        type="button"
        @else
        type="submit" @endif
                class="bg-red-700 hover:bg-red-800 text-white font-bold py-1 px-3 rounded text-sm main-button">Delete</button>
            ...
            <div x-show="showNotification" x-transition:enter="transition ease-out duration-300"
                ...
                x-transition:leave-end="opacity-0 transform translate-y-2" x-init="startNotificationTimer()"
                class="z-50 fixed bg-yellow-600 text-white py-2 px-4 rounded-xl bottom-3 left-3 text-sm">
                <p>This car has appointments and cannot be deleted</p>
            </div>
        </form>
