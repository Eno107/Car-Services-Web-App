<x-layout-simple>
    <div class="flex flex-col items-center justify-center mt-5 w-full">
        ...
        <div class=" flex items-center justify-center">
            <img src="/images/404image-removebg-preview.png" alt="" style="width: 80%">
        </div>

        <div class="flex flex-row items-center justify-center" style="color:#ed8554 ">
            <div id="container" style="width:33%">
                <div class="h-full flex flex-row items-center justify-end">
                    <div id="resizeText">4</div>
                </div>
            </div>

            <svg fill="#1F2937" version="1.1" style="width:32%;" id="Capa_1" xmlns="http://www.w3.org/2000/svg"
                xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 254.532 254.532" xml:space="preserve">
                <g>
                    <path
		...
		c-1.821,1.114-3.82,1.956-5.951,2.474v-0.072c-1.586,0.385-3.233,0.612-4.938,0.612S123.915,147.845,122.329,147.459z
		 ...
		 M179.292,172.23l-27.028-27.028c2.749-3.804,4.654-8.254,5.435-13.079h38.191C194.818,147.398,188.745,161.308,179.292,172.23z" />
            ...
            <div class="h-full flex flex-row items-center justify-start" style="width:33%">
                <div id="bottomResizeText">4</div>
            </div>
        </div>


        <div class="text-center text-lg sm:text-lg md:text-xl lg:text-xl xl:text-2xl">
            <h1><span class="main-color  text-3xl sm:text-3xl md:text-4xl lg:text-5xl xl:text-6xl">ERROR</span> the
                requested page does not exist.</h1>
        </div>

        <div class="mt-2">
            <a href="@auth /home @else / @endauth"
                class="underline main-color text-lg sm:text-lg md:text-xl lg:text-xl xl:text-2xl ">Go
                Back</a>
        </div>
    ...
    <script>
        function setFontSize() {
            ...
            var resizeText = document.getElementById('resizeText');
            var resizeSvg = document.getElementById('Capa_1');
            var bottomResizeText = document.getElementById('bottomResizeText');
            var containerWidth = container.clientWidth;
            var svgWidth = document.querySelector('svg').getBoundingClientRect().width;

            var maxWidth = Math.max(containerWidth, svgWidth);

            resizeText.style.fontSize = maxWidth + 'px';
            bottomResizeText.style.fontSize = maxWidth + 'px'
        }
