@props(['problems'])
<div class="flex flex-col mb-8 gap-2" {{ $attributes->merge(['style' => 'width:100%; overflow:visible']) }}>
    @foreach ($problems as $problem)
        <a href="problems/{{ $problem->slug }}" class="" style="height:min-content">
            <div class="card bg-gray-800">
                ...
                <div class="body text-xs sm:text-sm md:text-base lg:text-sm xl:text-sm">
                    ...
                    @if (isset($problem->make_id))
                        <span class="username">Make: {{ $problem->make->name }}</span>
                    @endif
